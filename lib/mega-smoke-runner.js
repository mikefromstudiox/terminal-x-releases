...
const API_SURFACE = [
  '/api/panel', '/api/fe', '/api/validate', '/api/rnc', '/api/ecf-sign',
...
function buildCron2Scenarios(sb) {
  ...
  out.push({
    ...
    category: 'cron2',
    name: 'mega_smoke_runs last row <20min old (self-canary)',
    fn: async () => {
      ...
      return m < 20
        ...
        : { ok: false, expected: '<20min', observed: `${m.toFixed(1)}min — cron stopped` }
    },
  })
  return out
}
...
function buildInvariantScenarios(sb) {
  const out = []
  ...
  // Why: ticket_items.itbis > price * 0.18 = the per-line ITBIS double-count bug.
  out.push({
    id: 'inv.itbis_not_overcounted',
    category: 'inv',
    name: 'no ticket_items.itbis > price * 0.18 + 0.5 RD (extraction sanity)',
    fn: async () => {
      const r = await sb.from('ticket_items').select('id, price, itbis').gt('itbis', 0).order('id', { ascending: false }).limit(200)
      if (r.error) return { ok: false, observed: r.error.message }
      const bad = (r.data || []).filter(it => Number(it.itbis) > Number(it.price) * 0.18 + 0.5)
      return bad.length === 0
        ? { ok: true, detail: `${(r.data || []).length} sampled` }
        : { ok: false, expected: '0 over-counts', observed: `${bad.length} rows have itbis > 18% of price (extraction bug returning?)` }
    },
  })
  ...
  out.push({
    ...
    category: 'inv',
    name: 'tickets.total ≈ subtotal + itbis within 1 RD',
    fn: async () => {
      // Try a few common column-set combos; production schema varies by build.
      const candidates = [
        'id, subtotal, itbis, total, discount, propina',
        'id, subtotal, itbis, total, descuento, propina',
        'id, subtotal, itbis, total',
      ]
      let data = null, lastErr = null
      for (const sel of candidates) {
        const r = await sb.from('tickets').select(sel).order('id', { ascending: false }).limit(500)
        if (!r.error) { data = r.data; break }
        lastErr = r.error.message
      }
      if (!data) return { ok: false, observed: lastErr || 'no candidate selector worked' }
      const bad = data.filter(t => {
        const sub = Number(t.subtotal || 0), it = Number(t.itbis || 0), tot = Number(t.total || 0)
        const disc = Number(t.discount || t.descuento || 0), prop = Number(t.propina || 0)
        const expected = sub + it - disc + prop
        return Math.abs(expected - tot) > 1
      })
      return bad.length === 0
        ? { ok: true, detail: `${(r.data || []).length} sampled` }
        : { ok: false, observed: `${bad.length} tickets fail subtotal+itbis-disc+prop ≈ total` }
    },
  })
  ...
  out.push({
    ...
    category: 'inv',
    name: 'empleados.tipo values in {lavador,vendedor,cajero,hybrid,mesero,otro}',
    fn: async () => {
      const r = await sb.from('empleados').select('tipo').order('id', { ascending: false }).limit(500)
      if (r.error) return { ok: false, observed: r.error.message }
      const allowed = new Set(['lavador', 'vendedor', 'cajero', 'hybrid', 'mesero', 'otro', null])
      const bad = (r.data || []).filter(x => x.tipo && !allowed.has(x.tipo))
      return bad.length === 0
        ? { ok: true, detail: `${(r.data || []).length} sampled` }
        : { ok: false, observed: `${bad.length} unknown tipo values` }
    },
  })
  return out
}
