    ...
    out.push({
      ...
      fn: async () => {
        ...
        if (c.path === '/api/panel?action=cron_deploy_smoke')      { table = 'deploy_smoke_results'; extra = { col: 'source', val: 'cron' } }
          ...
          // No-op early exit: if no business has daily_digest_enabled, the
          // cron correctly writes nothing. Skip rather than fail.
          const { count: enabledCount } = await sb.from('app_settings')
            .select('business_id', { count: 'exact', head: true })
            .eq('key', 'daily_digest_enabled')
            .in('value', ['1', 'true', 'TRUE'])
          if (!enabledCount) return { ok: true, skip: true, detail: 'no businesses with daily_digest_enabled — nothing to send' }
          table = 'activity_log'; dateCol = 'created_at'; extra = { col: 'event_type', val: 'daily_digest_sent' }
        }
