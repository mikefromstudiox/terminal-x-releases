...
export function createHarness(opts = {}) {
  ...
  // 2026-05-19 — added retry+spread for parallel suite runs. Management API
  // hits 60 req/min cap when multiple suites run concurrently under
  // pre-release.mjs. Earlier behavior: throw on first 429 → scenario fails
  // with 'ThrottlerException' instead of revealing real findings.
  let _lastPgAt = 0
  async function pgQuery(sql) {
    ...
    // Spread requests to keep parallel suites under 60 req/min.
    const now = Date.now()
    const dt = now - _lastPgAt
    if (dt < 200) await new Promise(r => setTimeout(r, 200 - dt))
    _lastPgAt = Date.now()
    const RETRIES = [0, 800, 2000, 4000]
    let lastBody = ''
    for (let i = 0; i < RETRIES.length; i++) {
      ...
      lastBody = (await r.text()).slice(0, 240)
      // Retry on 429/503; everything else fails fast.
      if (r.status !== 429 && r.status !== 503) {
        throw new Error(`pgQuery ${r.status}: ${lastBody}`)
      }
      _lastPgAt = Date.now()
    }
    throw new Error(`pgQuery 429 (exhausted ${RETRIES.length} retries): ${lastBody}`)
  }
