...
- **Owner Activity Feed (2026-04-13):** Append-only audit log covering user deletions, service deletions/price changes, ticket voids, notas de crédito, nómina payouts, big discounts (>RD$500 or >15%), inventory adjustments, caja chica withdrawals, cuadre discrepancies (>RD$50). Table `activity_log` (SQLite + Supabase) synced via `electron/sync.js` (FWW append-only; `metadata` is a JSON column). Helpers in `electron/database.js`: `setActiveUser({id,name,role})` (called from `AuthContext` on login), `activityLogRecord({event_type,severity,target_type,target_id,target_name,amount,old_value,new_value,reason,metadata})`, `activityLogList({dateFrom,dateTo,eventTypes,limit})`. IPC: `activity:set-actor`, `activity:list`. Web path writes directly to Supabase via `logActivity()` helper in `packages/data/web.js`. UI: new **Actividad** tab on `RemoteDashboard.jsx` (owner/cfo/accountant only) with 9 filter chips (Todo/Eliminaciones/Precios/Anulaciones/Pagos/Descuentos/Inventario/Caja Chica/Cuadre), 30-day window, severity rail (info/warn/critical → slate/amber/red), expandable rows showing old→new diff + metadata JSON. When adding new audited events: call `activityLogRecord` at the mutation site, add event_type to `EVENT_META` + a filter chip in `RemoteDashboard.jsx`. Never call INSERT INTO activity_log from new endpoints — always route through the helper.
