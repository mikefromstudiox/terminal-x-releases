...
Total tables: **150** (RLS enabled: **150**)
...
- Rough row count (n_live_tup): **131**
- RLS enabled: **YES**
...
- Rough row count (n_live_tup): **1167**
- RLS enabled: **YES**
...
- Rough row count (n_live_tup): **159**
- RLS enabled: **YES**
...
- Policies referencing `app_metadata`: **202** (CORRECT)
...
```sql
((business_id IS NOT NULL) AND ((business_id)::text = COALESCE(((current_setting('request.jwt.claims'::text, true))::jsonb ->> 'business_id'::text), ''::text)))
```
...
```sql
(business_id = (COALESCE(((current_setting('request.jwt.claims'::text, true))::json ->> 'business_id'::text), ((current_setting('request.headers'::text, true))::json ->> 'x-business-id'::text)))::uuid)
```
...
| `tickets` | `trg_tickets_complete_appointment` | AFTER | UPDATE,INSERT | EXECUTE FUNCTION trg_ticket_complete_appointment() |  |
...
| `washer_commissions` | `trg_washer_comm_employment_window` | BEFORE | INSERT,UPDATE | EXECUTE FUNCTION guard_commission_employment_window() |  |
...
- Tables: **150** (RLS-enabled: 150)
- Columns: **2987**
- Constraints: **740** (PK: 182, UNIQUE: 278, FK: 153, CHECK: 125)
- Indexes: **939** (partial: 129)
- Policies: **408** (`app_metadata`: 202, `user_metadata`: 0)
- Functions: **244**
- Triggers: **273**
- Realtime members: **24**
...
| date | who | summary |
|------|-----|---------|
| 2026-05-01 | dataLEAKS | initial snapshot |
