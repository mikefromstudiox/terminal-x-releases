...
function AdminLogin({ onSuccess }) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 p-8">
      <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8 w-full max-w-sm space-y-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-sky-100 rounded-xl flex items-center justify-center">
            <Lock size={18} className="text-sky-600" />
          </div>
          <div>
            <h3 className="text-[15px] font-bold text-slate-800">Acceso Admin</h3>
            <p className="text-[12px] text-slate-400">Clave de administrador del servidor</p>
          </div>
        </div>
        ...
        <input type="password" value={adminKey}
          ...
          autoFocus
          className="w-full px-4 py-3 border border-slate-200 rounded-xl text-[13px] focus:outline-none focus:border-sky-400 bg-slate-50" />
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-6">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h3 className="text-[15px] font-bold">Nueva Licencia</h3>
        ...
        <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          {[
            ...
            <div key={f.k}>
              <label className="block text-[11px] font-semibold text-slate-500 mb-1">{f.label} <span className="text-red-400">*</span></label>
              <input type="text" value={form[f.k]} onChange={e => set(f.k, e.target.value)} placeholder={f.ph}
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[13px] focus:outline-none focus:border-sky-400" />
            </div>
          ))}
              ...
              <div key={f.k}>
                ...
                {f.type === 'select'
                  ? <select value={form[f.k]} onChange={e => set(f.k, e.target.value)}
                      className="w-full px-2 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[12px] focus:outline-none focus:border-sky-400">
                      {f.opts.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                    </select>
                  : <input type="number" value={form[f.k]} onChange={e => set(f.k, e.target.value)} min="1"
                      className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[12px] focus:outline-none focus:border-sky-400" />
                }
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={e => set('notes', e.target.value)} rows={2}
              className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[12px] focus:outline-none focus:border-sky-400 resize-none" />
          </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 border border-slate-200 text-slate-600 rounded-xl text-[13px] font-semibold hover:bg-slate-50">Cancelar</button>
...
function LicenseRow({ license, adminKey, onRefresh }) {
  ...
  return (
    <div className={`border-b border-slate-100 ${expanded ? 'bg-slate-50' : 'hover:bg-slate-50/50'}`}>
      <button className="w-full flex items-center px-5 h-14 gap-4 text-left" onClick={() => setExpanded(e => !e)}>
        <span className={`w-2 h-2 rounded-full shrink-0 ${sc.dot}`} />
        <div className="flex-1 min-w-0">
          <p className="text-[13px] font-semibold text-slate-800 truncate">{license.business_name}</p>
          <p className="text-[11px] text-slate-400">{license.business_rnc}</p>
        </div>
        ...
        <div className="px-5 pb-4 pt-3 border-t border-slate-100 space-y-3">
          <div className="grid grid-cols-4 gap-4 text-[12px]">
            {[
              ...
              <div key={lbl}><p className="text-slate-400 mb-0.5">{lbl}</p><p className="font-medium text-slate-700">{val}</p></div>
            ))}
          </div>

          {license.hardware_id && (
            <div className="bg-slate-100 rounded-lg px-3 py-2">
              <p className="text-[10px] text-slate-400 mb-0.5">ID de Equipo</p>
              <code className="text-[11px] text-slate-600 font-mono break-all">{license.hardware_id}</code>
            </div>
          )}
          {license.notes && (
...
export default function LicenseAdmin() {
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      {/* Stats */}
      <div className="shrink-0 px-6 py-4 border-b border-slate-100 grid grid-cols-4 gap-4">
        {[
          ...
          <div key={lbl} className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-center">
      ...
      <div className="shrink-0 px-6 py-3 border-b border-slate-100 flex items-center gap-3">
        <div className="flex-1 relative">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[13px] focus:outline-none focus:border-sky-400" />
        </div>
        <a href={`${LICENSE_API}/admin`} target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-1.5 px-3 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 text-[12px] font-medium rounded-xl transition-colors">
          <ExternalLink size={13} /> Panel Web
        </a>
        <button onClick={loadLicenses} disabled={loading}
          className="w-9 h-9 flex items-center justify-center text-slate-400 hover:text-sky-600 hover:bg-sky-50 rounded-xl">
      ...
      {/* Column headers */}
      <div className="shrink-0 flex items-center h-9 bg-slate-50 border-b border-slate-200 px-5 text-[10px] font-bold text-slate-400 uppercase tracking-wider gap-4">
      ...
      <div className="flex-1 overflow-y-auto">
        {error ? (
          ...
          Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="flex items-center h-14 border-b border-slate-100 px-5 gap-4 animate-pulse">
              <div className="w-2 h-2 bg-slate-100 rounded-full" />
              <div className="flex-1 space-y-1.5"><div className="h-3.5 bg-slate-100 rounded w-3/4" /><div className="h-3 bg-slate-100 rounded w-1/3" /></div>
              <div className="w-40 h-4 bg-slate-100 rounded" /><div className="w-16 h-5 bg-slate-100 rounded-full" />
            </div>
          ))
        ) : filtered.length === 0 ? (
  ...
  return (
    <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-slate-200">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-[16px] font-bold text-slate-800">Licencias TX</h2>
          <p className="text-[11px] text-slate-400">Solo para administradores Terminal X</p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        {hwid && <p className="text-[10px] text-slate-400"><span className="font-medium">HWID:</span> <code>{hwid?.slice(0,8)}…</code></p>}
        {onLogout && (
          <button onClick={onLogout}
            className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-200 text-slate-600 hover:bg-slate-50 text-[12px] font-medium rounded-xl">
