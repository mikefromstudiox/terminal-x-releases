...
export function createWebAPI(supabase, businessId) {
  ...
  const api = {
    ...
    tickets: {
      ...
      // 2026-05-17 — Mesas add-on running-tab append flow. Loads the latest
      // open ticket for a given mesa (by supabase_id) plus all its
      // ticket_items so the POS can re-hydrate them into the cart for an
      // add-on order. Verified against pg_catalog 2026-05-17: tickets has
      // (id uuid, supabase_id uuid, mesa_supabase_id uuid, open_status text);
      // ticket_items has (supabase_id uuid, ticket_supabase_id uuid, name,
      // price, quantity, preparation_notes, weight, service_supabase_id,
      // inventory_item_supabase_id, is_wash, itbis, cost). aplica_itbis
      // lives on services, not ticket_items.
      byMesa: (mesaSupabaseId) => tryOr(async () => {
        ...
        const { data: ticket } = await supabase.from('tickets')
          ...
          .limit(1)
          .maybeSingle()
        if (!ticket || !ticket.supabase_id) return null
        const { data: rows } = await supabase.from('ticket_items')
        ...
        const items = (rows || []).filter(i => i && i.name != null).map(i => ({
          ...i,
          qty: i.quantity,
      ...
      // 2026-05-17 — Mesas add-on. Append NEW items to an already-open ticket
      // (mesa running tab). Recomputes subtotal/itbis/total from existing +
      // new items using per-business ITBIS rate (gross→net extraction —
      // matches commit b3b1672 "per-item ITBIS extraction from gross price").
      // Throws loudly if the ticket has been closed/voided between load and
      // save (race against another cashier). Activity-logs ticket_append_items.
      appendItems: ({ ticketId, ticketSupabaseId, items, workers }) => tryWrite(async () => {
        const tSid = ticketSupabaseId
        if (!tSid) throw new Error('Falta ticket_supabase_id')
        if (!Array.isArray(items) || items.length === 0) throw new Error('No hay items para agregar')

        // Pre-check ticket is still open. Use supabase_id (canonical) — id is
        // uuid on Supabase and may differ from the local int.
        const { data: cur } = await supabase.from('tickets')
          .select('id, supabase_id, open_status, status, subtotal, itbis, total, mesa_supabase_id, doc_number, rev')
          .eq('business_id', bid)
          .eq('supabase_id', tSid)
          .maybeSingle()
        ...
        // Resolve per-business ITBIS rate once.
        ...
        // Look up aplica_itbis flags for service items (mirrors tickets.create).
        const svcIds = items.map(i => i.service_id).filter(Boolean)
        let svcItbisById = new Map()
        let svcCostById = new Map()
        if (svcIds.length) {
          const { data: svcRows } = await supabase.from('services')
            .select('id, cost, aplica_itbis').in('id', svcIds)
          svcItbisById = new Map((svcRows || []).map(r => [r.id, r.aplica_itbis ?? 1]))
          svcCostById  = new Map((svcRows || []).map(r => [r.id, r.cost || 0]))
        }

        // Build new ticket_items rows with deterministic supabase_id each.
        ...
        if (ins.error) {
          // Retry without business_id (older schemas)
          const stripped = newRows.map(({ business_id, ...rest }) => rest)
          const retry = await supabase.from('ticket_items').insert(stripped)
          if (retry.error) throw new Error(retry.error.message || 'ticket_items insert failed')
        }
        ...
        const total = subtotal // gross convention — ITBIS already embedded in price

        // Update ticket totals only — DO NOT touch status/open_status/mesa.
        const upd = await supabase.from('tickets').update({
          subtotal, itbis, total, updated_at: new Date().toISOString(),
        }).eq('business_id', bid).eq('supabase_id', tSid)
        if (upd.error) throw new Error(upd.error.message || 'tickets update failed')

        const addedTotal = newRows.reduce((s, r) => s + (Number(r.price) || 0) * (Number(r.quantity) || 1), 0)
        try {
          await logActivity({
          ...
          try { window.__txReportError?.(e, { severity: 'warn', category: 'tickets.appendItems.activityLog' }) } catch {}
        }
