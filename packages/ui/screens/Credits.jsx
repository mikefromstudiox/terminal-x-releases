...
function CreditBar({ balance, limit, height = 'h-1.5' }) {
  ...
  return (
    <div className={`w-full bg-slate-100 dark:bg-white/10 rounded-full ${height} overflow-hidden`}>
...
function ClientCard({ client, selected, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left px-4 py-3.5 border-b border-slate-100 dark:border-white/10 transition-colors ${
        selected ? 'bg-[#f0f6ff] dark:bg-blue-500/10 border-l-2 border-l-[#378ADD]' : 'hover:bg-slate-50 dark:hover:bg-white/10'
      }`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {client.rnc   && <p className="text-[11px] text-slate-400 dark:text-white/40 mt-0.5">RNC: {client.rnc}</p>}
          {client.phone && <p className="text-[11px] text-slate-400 dark:text-white/40">{client.phone}</p>}
          <div className="mt-1.5">
            <CreditBar balance={client.balance} limit={client.credit_limit} />
            <div className="flex items-center justify-between mt-0.5">
              <p className="text-[10px] text-slate-500 dark:text-white/60 font-medium">{fmtRD(client.balance)} adeudado</p>
              <p className="text-[10px] text-slate-400 dark:text-white/40">{pct}%</p>
            </div>
          </div>
        </div>
      </div>
...
function PaymentForm({ total, maxAmount, clientRnc, onPay, paying, isManual = false }) {
  ...
  return (
    <div className="border-t border-slate-100 dark:border-white/10 pt-4 space-y-3">
      ...
      {isManual && (
        <div>
          <label className="block text-[11px] font-semibold text-slate-500 dark:text-white/60 mb-1">Monto a abonar (RD$)</label>
          <input
            type="number"
            min="0"
            step="0.01"
        ...
        {['E32','E31'].map(t => (
          <button key={t} onClick={() => setNcfType(t)}
            className={`flex-1 py-1.5 rounded-lg text-[12px] font-semibold border transition-colors ${
              ncfType === t
                ? 'bg-[#E6F1FB] dark:bg-blue-500/20 border-[#0C447C] text-[#0C447C] dark:text-blue-300'
                : 'bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-500 dark:text-white/60 hover:border-slate-300 dark:hover:border-white/20'
            }`}>
      ...
      {ncfType === 'E31' && (
        <div>
          <label className="block text-[11px] font-semibold text-slate-500 dark:text-white/60 mb-1">RNC del cliente</label>
      ...
      {/* Comentario */}
      <div>
        <label className="block text-[11px] font-semibold text-slate-500 dark:text-white/60 mb-1">Comentario (opcional)</label>
...
function ClientDetail({ client, onReload }) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative">
      {/* Toast */}
      {toast && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-40 bg-slate-800 dark:bg-white/90 text-white dark:text-black text-[12px] font-medium px-4 py-2 rounded-xl shadow-lg whitespace-nowrap">
          {toast}
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-[16px] font-bold text-slate-800 dark:text-white leading-tight">{client.name}</h2>
            {client.rnc && <p className="text-[12px] text-slate-400 dark:text-white/40">RNC: {client.rnc}</p>}
          </div>
        </div>

        {/* Contact */}
        <div className="space-y-1.5">
          {client.phone   && <div className="flex items-center gap-2 text-[12px] text-slate-600 dark:text-white/60"><Phone size={12} className="text-slate-400 dark:text-white/40 shrink-0" />{client.phone}</div>}
          {client.email   && <div className="flex items-center gap-2 text-[12px] text-slate-600 dark:text-white/60"><Mail  size={12} className="text-slate-400 dark:text-white/40 shrink-0" />{client.email}</div>}
          {client.address && <div className="flex items-center gap-2 text-[12px] text-slate-600 dark:text-white/60"><MapPin size={12} className="text-slate-400 dark:text-white/40 shrink-0" />{client.address}</div>}
        </div>

        {/* Notes */}
        {client.notes && (
          <div className="flex items-start gap-2 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl px-3 py-2.5">
            <StickyNote size={13} className="text-amber-500 shrink-0 mt-0.5" />
            <p className="text-[12px] text-amber-800 dark:text-amber-300">{client.notes}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={s.label} className="bg-slate-50 dark:bg-white/5 rounded-xl px-3 py-2.5 text-center">
              <p className="text-[13px] font-bold text-slate-800 dark:text-white leading-tight">{s.value}</p>
              <p className="text-[10px] text-slate-400 dark:text-white/40 mt-0.5">{s.label}</p>
            </div>
          ))}
        </div>

        {/* Credit block */}
        <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3.5 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-slate-500 dark:text-white/60 uppercase tracking-wider">Crédito</span>
            <span className="text-[11px] text-slate-400 dark:text-white/40">Límite: {fmtRD(client.credit_limit)}</span>
          </div>
          <CreditBar balance={client.balance} limit={client.credit_limit} height="h-2" />
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[18px] font-black text-slate-800 dark:text-white">{fmtRD(client.balance)}</p>
              <p className="text-[11px] text-slate-400 dark:text-white/40">Balance adeudado</p>
            </div>
            <div className="text-right">
              <p className={`text-[15px] font-bold ${overLimit ? 'text-red-500' : 'text-emerald-600'}`}>
                {fmtRD(available)}
              </p>
              <p className="text-[11px] text-slate-400 dark:text-white/40">Disponible</p>
            </div>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            {tickets.length > 0 && (
              <button onClick={toggleAll}
                className="flex items-center gap-1 text-[11px] font-semibold text-[#0C447C] dark:text-blue-400 hover:underline">
                <div className={`w-3.5 h-3.5 rounded border-2 flex items-center justify-center transition-colors ${
                  allChecked ? 'bg-[#0C447C] border-[#0C447C]' : 'border-slate-300 dark:border-white/30'
                }`}>
          ...
          {loading ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="text-[13px] text-slate-500 dark:text-white/60 font-medium">No hay tickets pendientes</p>
              <p className="text-[11px] text-slate-400 dark:text-white/40">Esta cuenta está al día</p>
            </div>
          ) : (
              ...
              {tickets.map(t => (
                <button key={t.id} onClick={() => toggleOne(t.id)}
                  className={`w-full rounded-xl border text-left transition-colors overflow-hidden ${
                    checked.has(t.id)
                      ? 'bg-[#E6F1FB] dark:bg-blue-500/10 border-[#0C447C]/40'
                  ...
                  <div className="flex items-center gap-3 px-3 pt-3 pb-2">
                    <div className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${
                      checked.has(t.id) ? 'bg-[#0C447C] border-[#0C447C]' : 'border-slate-300 dark:border-white/30'
                    }`}>
                      {checked.has(t.id) && <Check size={9} className="text-white" strokeWidth={3} />}
                    </div>
                    <p className="text-[12px] font-bold text-slate-700 dark:text-white flex-1">{t.doc_number}</p>
                    <p className="text-[10px] text-slate-400 dark:text-white/40 shrink-0">{fmtDate(t.created_at)}</p>
                  </div>

                  {/* Item lines */}
                  {t.items && t.items.length > 0 ? (
                    <div className="px-3 pb-2.5 space-y-1 border-t border-slate-100 dark:border-white/10 pt-2">
                      {t.items.map((item, i) => (
                        <div key={i} className="flex items-center justify-between">
                          <span className="text-[11px] text-slate-600 dark:text-white/60 truncate pr-2">{item.name}</span>
                          <span className="text-[11px] font-semibold text-slate-700 dark:text-white shrink-0 tabular-nums">{fmtRD(item.price)}</span>
                        </div>
                      ))}
                      {/* Ticket subtotal */}
                      <div className="flex items-center justify-between pt-1 mt-1 border-t border-slate-200 dark:border-white/10">
                        <span className="text-[10px] text-slate-400 dark:text-white/40">
                          {t.vehicle_plate ? `Vehículo: ${t.vehicle_plate}` : 'Al portador'}
                        </span>
                        <span className="text-[12px] font-black text-slate-800 dark:text-white tabular-nums">{fmtRD(t.total)}</span>
                      </div>
                    </div>
                  ) : (
                    <div className="px-3 pb-2.5 flex items-center justify-between border-t border-slate-100 dark:border-white/10 pt-2">
                      <span className="text-[11px] text-slate-400 dark:text-white/40">{t.vehicle_plate || 'Al portador'}</span>
                      <span className="text-[12px] font-black text-slate-800 dark:text-white">{fmtRD(t.total)}</span>
                    </div>
                  )}
                </button>
              ))}
        ...
        {checked.size > 0 && (
          <div className="bg-white dark:bg-white/5 border border-[#0C447C]/20 rounded-xl overflow-hidden">
            {/* Header */}
            <div className="bg-[#f0f6ff] dark:bg-blue-500/10 px-4 py-2.5 flex items-center justify-between">
              <p className="text-[11px] font-bold text-[#0C447C] dark:text-blue-300 uppercase tracking-wider">
            ...
            <div className="px-4 py-3 space-y-3">
              {tickets.filter(t => checked.has(t.id)).map((t, idx, arr) => (
                <div key={t.id}>
                  {/* Ticket label */}
                  <p className="text-[10px] font-bold text-slate-400 dark:text-white/40 uppercase mb-1.5">
                  ...
                  {(t.items || []).map((item, i) => (
                    <div key={i} className="flex items-center justify-between py-0.5">
                      <span className="text-[12px] text-slate-700 dark:text-white truncate pr-3">{item.name}</span>
                      <span className="text-[12px] font-semibold text-slate-700 dark:text-white tabular-nums shrink-0">{fmtRD(item.price)}</span>
                    </div>
                  ))}
                  {/* Ticket total */}
                  <div className="flex items-center justify-between mt-1 pt-1 border-t border-slate-100 dark:border-white/10">
                    <span className="text-[11px] text-slate-400 dark:text-white/40">Subtotal ticket</span>
                    <span className="text-[12px] font-bold text-slate-700 dark:text-white tabular-nums">{fmtRD(t.total)}</span>
                  </div>
                  {idx < arr.length - 1 && <div className="border-t border-dashed border-slate-200 dark:border-white/10 mt-3" />}
                </div>
              ))}

              {/* Grand total */}
              <div className="border-t-2 border-slate-800 dark:border-white pt-2 flex items-center justify-between">
                <p className="text-[13px] font-bold text-slate-800 dark:text-white">Total a Cobrar</p>
                <p className="text-[18px] font-black text-slate-800 dark:text-white tabular-nums">{fmtRD(selectedTotal)}</p>
              </div>
...
export default function Credits() {
  ...
  return (
    <div className="h-full flex flex-col md:flex-row bg-white dark:bg-black">

      {/* ── Left panel — client list (hidden on mobile when detail is open) ── */}
      <div className={`border-r border-slate-100 dark:border-white/10 flex flex-col ${
        selectedClient ? 'hidden md:flex w-full md:w-[300px] md:shrink-0' : 'flex flex-1'
      }`}>

        {/* Header */}
        <div className="shrink-0 px-3 py-2.5 md:px-4 md:py-3 border-b border-slate-200 dark:border-white/10">
          <div className="flex items-center justify-between mb-2.5">
            <h2 className="text-[14px] md:text-[16px] font-bold text-slate-800 dark:text-white">Cuentas x Cobrar</h2>
            <button
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {selectedClient ? (
          <>
            {/* Mobile back button */}
            <div className="md:hidden shrink-0 flex items-center gap-2 px-3 py-2 border-b border-slate-100 dark:border-white/10">
              <button
                onClick={() => setSelectedId(null)}
                className="flex items-center gap-1.5 text-[12px] font-medium text-slate-500 dark:text-white/60 hover:text-slate-700 dark:hover:text-white min-h-[44px]"
              >
          ...
          <div className="hidden md:flex flex-1 flex-col items-center justify-center text-center px-8">
            <div className="w-14 h-14 bg-slate-50 dark:bg-white/5 rounded-2xl flex items-center justify-center mb-4">
              <CreditCard size={24} className="text-slate-300 dark:text-white/30" />
            </div>
            <p className="text-[14px] font-semibold text-slate-500 dark:text-white/60">Selecciona un cliente</p>
            <p className="text-[12px] text-slate-400 dark:text-white/40 mt-1">
