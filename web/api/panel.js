...
//
// Cost ballpark: Haiku 4.5 ≈ $0.0005/triage × ≤5/run × 30 runs/h × 24h ≈ $1.80/day.
...
async function _triageGetRecentGit() {
  // Vercel doesn't ship .git, so we fall back to VERCEL_GIT_COMMIT_SHA +
  // VERCEL_GIT_COMMIT_MESSAGE for current deploy context. Best-effort only.
...
async function _triageCallClaude(prompt) {
  ...
  try {
    ...
    // Try to parse a JSON object out of the response — model is asked for
    // pure JSON but might wrap in fences or chatter. Best-effort extract.
...
async function handleCronClaudeTriage(req, res) {
  ...
  try {
    ...
    // Gather context once per run (shared across all diagnoses)
    ...
    const queue = [] // { kind, row, tableName, idCol, applyDiagnosis }

    // 1) Critical client_errors not yet diagnosed (lookback 15min, cap 5)
      ...
      queue.push({
        ...
        async applyDiagnosis(diagnosis) {
          ...
          if (!wa.sent && wa.reason) {
            // Make the gap visible — single warn row, deduped by reportServerError's _recent set.
            await sb.from('client_errors').insert({
    ...
    // 2) deploy_smoke_results failures not yet diagnosed
...
// ─────────────────────────────────────────────────────────────────────────────
// Imports the runner as an ES module and runs it server-side. Same auth +
// escalation pattern as cron_deploy_smoke (Layer 1) and cron_health_verifier
// (Layer 3). The base URL is read from VERCEL_URL for self-targeting; falls
// back to terminalxpos.com so CRON_SECRET-triggered local runs hit prod.
async function handleCronFlowDriftSmoke(req, res) {
  ...
  try {
    // Runner is in ../lib/flow-drift-runner.js — repo root mirrors
    // web/lib/, and prepare-vercel.mjs copies web/lib/*.js → dist-web/lib/
    // so the same relative path resolves in the deployed function bundle.
